-- Up Migration
-- Add human-readable timestamp columns for quiz_sessions (BIGINT milliseconds)
...
ALTER TABLE quiz_sessions ADD COLUMN IF NOT EXISTS ended_at_readable TIMESTAMPTZ GENERATED ALWAYS AS (CASE WHEN ended_at IS NOT NULL THEN to_timestamp(ended_at::double precision / 1000) ELSE NULL END) STORED;

-- Add human-readable timestamp column for guesses (BIGINT milliseconds)
...
CREATE INDEX IF NOT EXISTS quiz_sessions_ended_at_readable_idx ON quiz_sessions(ended_at_readable);
...
-- DROP INDEX IF EXISTS quiz_sessions_ended_at_readable_idx;
